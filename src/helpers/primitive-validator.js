...
const validatePopulated = function(value) {
...
const validateFormat = function(fhirType, value) {
	const validator = validators[fhirType] || validators["string"];
	if (!validator[0].test(value)) { return validator[1]; }
};

export default PrimitiveValidator = function(fhirType, value, failBlank) {
	let badFormat;
